...
<!-- GENERATED by `bun packages/prx/scripts/gen-value-props.ts` by EXECUTING the forcing-function checks in packages/prx/src/value_props.ts. A claim is BACKED only if its checks pass right now — it cannot overclaim. Drift-checked by packages/prx/test/value_props.test.ts. Do not edit by hand. -->
...
- ✅ a push produced by reviewer is rejected
...
- ✅ submit accepts a beads canonical id (not GH-only)
  - exercises: submit/artifact.schema.ts:submitRefFor, submit/artifact.schema.ts:WORK_UNIT_RE
- 📎 driven end-to-end to a merged PR — prx-2c4 → PR #85 (merged); chain source@pinned→plan→implement on disk; prx publisher did the merge
