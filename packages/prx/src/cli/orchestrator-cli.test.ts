...
import { runSpecVerb } from "./orchestrator-cli.ts";
...
describe("runSpecVerb — the `prx pilot`/`prx fleet` bridge", () => {
  test("pilot drives the (stub) pipeline to merged and exits 0", async () => {
    ...
    const code = await runSpecVerb("pilot", ["GH-9"], out);
    expect(code).toBe(0);
  ...
  test("fleet runs over comma-split units and exits 0", async () => {
    const { out, lines } = sink();
    const code = await runSpecVerb("fleet", ["a,b,c"], out);
    expect(code).toBe(0);
  ...
  test("a missing required arg surfaces an error and exits 1", async () => {
    const { out, lines } = sink();
    const code = await runSpecVerb("pilot", [], out);
    expect(code).toBe(1);
    expect(lines.join("\n")).toContain("ERR:");
  });
});
