...
export const usagePlanTiers = ["free", "pro", "max", "team", "enterprise", "api", "unknown"] as const;
...
export const usageStateV1Schema = z.object({
  ...
  apiRateLimits: z.array(z.object({
...
type UsageRawInput = Pick<
  UsageStateV1,
  "session" | "weekly" | "capacity" | "apiRateLimits"
>;
...
function collectDimensionFractions(raw: UsageRawInput): DimensionFraction[] {
  ...
  if (contextWindowTokens !== null && currentConversationTokens !== null && contextWindowTokens > 0) {
    entries.push({
      dimension: "capacity",
