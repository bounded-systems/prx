...
export const GcClass = z.enum(["orphan", "stale", "drift", "missed-close", "dup"]);
...
export const TeardownOutput = z.object({
  unit: z.string(),
  status: z.enum(["torn-down", "would-tear-down", "not-found", "partial", "error"]),
  /** Footprint classes removed (or, in dry-run, that would be removed). */
  removed: z.array(z.enum(["worktree", "beads", "branch", "gh-verified"])),
  error: z.string().optional(),
});
