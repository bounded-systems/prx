 ...
 * unioned with the capabilities carried by every **consumed** door. Exposed
 * doors are services the room offers, NOT occupant grants, so they are excluded
 * — a build room that *exposes* `nix:build` does not thereby let an arbitrary
 * occupant invoke it; the door is how *other* rooms reach it. Sorted + deduped
 * so the boundary is a stable set.
 */
export function roomGrants(room: RoomSpec): string[] {
  const parsed = RoomSpecSchema.parse(room);
  const consumed = parsed.doors
    .filter((d) => d.direction === "consume")
    .map((d) => d.capability);
  return [...new Set([...parsed.grants, ...consumed])].sort();
}

/** True iff the room exposes a door carrying `capability` (a service it offers). */
export function roomExposes(room: RoomSpec, capability: string): boolean {
  return RoomSpecSchema.parse(room).doors.some(
    (d) => d.direction === "expose" && d.capability === capability,
  );
}
