...
export function renderPodmanKube(pod: PodSpec): string {
  ...
  for (const room of p.rooms) {
    ...
    // "$PRX_BEADS_SOCKET"'` (the consumer env `podRoomEnv` already projects).
    const cmdArgs = [...socketArgs, ...room.extraArgs];
    if (cmdArgs.length > 0) {
      lines.push("      args:");
      for (const arg of cmdArgs) lines.push(`        - ${dq(arg)}`);
    }
  }
...
/** The systemd Quadlet `.kube` unit filename for a pod (drop into
 *  `containers/systemd/`, alongside the rendered YAML it wraps). */
export function quadletKubeUnitName(pod: PodSpec): string {
  return `${PodSpecSchema.parse(pod).name}.kube`;
}

/**
 * Render the **production** runtime form of the whole kube-play pod (prx-8uf2):
 * a systemd Quadlet `.kube` unit wrapping the {@link renderPodmanKube} manifest.
 * Where {@link renderPodmanQuadlet} is the durable counterpart of a single
 * secret room's ad-hoc `podman run`, this is the durable counterpart of
 * `playPod`'s ad-hoc `podman kube play -` — the whole non-secret pod gets
 * systemd's restart-on-failure and boot-time bring-up instead of a one-shot
 * CLI invocation with no lifecycle.
 *
 * `PublishPort=` is emitted for every non-secret room that declares `tcpPort`
 * (e.g. a door-bridge room, prx-8uf2) — the kube YAML itself has no `ports:`
 * field (Podman's `.kube` Quadlet docs: `PublishPort=` in the unit is merged
 * with the YAML's own ports), so this is the ONLY place a kube room's TCP
 * port actually gets published to the host. Always loopback-bound
 * (`127.0.0.1:<port>:<port>`) — same off-host-credential-leak reasoning as
 * {@link renderPodmanRun}'s `--publish`.
 *
 * `yamlPath` is the path the unit's `Yaml=` key points at — the caller writes
 * {@link renderPodmanKube}'s output there (this function does not write
 * files; same "render only" contract as {@link renderPodmanQuadlet}).
 */
export function renderPodmanKubeQuadlet(pod: PodSpec, yamlPath: string): string {
  const p = PodSpecSchema.parse(pod);
  const publishPorts = p.rooms
    .filter((r) => !roomNeedsSecretRuntime(r) && r.tcpPort !== undefined)
    .map((r) => `PublishPort=127.0.0.1:${r.tcpPort}:${r.tcpPort}`);

  const lines: string[] = [
    "# Generated by prx (pod spec → podman Quadlet .kube unit, prx-8uf2). Do not hand-edit.",
    `# Install: cp ${quadletKubeUnitName(p)} <yaml> ~/.config/containers/systemd/`,
    `#          systemctl --user daemon-reload && systemctl --user enable --now ${p.name}`,
    "[Unit]",
    `Description=prx pod ${p.name} (prx-asr / prx-8uf2)`,
    "",
    "[Kube]",
    `Yaml=${yamlPath}`,
    ...publishPorts,
    ...
    "[Service]",
    "Restart=on-failure",
    // Image pulls can exceed systemd's default unit start timeout.
    "TimeoutStartSec=900",
    "",
    "[Install]",
