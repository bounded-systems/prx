...
describe("readUnitTelemetry", () => {
  test("returns only TELEMETRY_* rows for the requested unit, in order", () => {
    const lines = [
      row({ ts: "t1", event: "TELEMETRY_SEAM_OBSERVED", workUnitId: "GH-1", details: { seam: "intake", phase: "start" } }),
      ...
      row({ ts: "t4", event: "TELEMETRY_SEAM_OBSERVED", workUnitId: "GH-1", details: { seam: "intake", phase: "done", elapsedMs: 9 } }),
    ];
  ...
  test("skips malformed/partial lines instead of throwing", () => {
    ...
    const events = readUnitTelemetry(opts("GH-1", lines));
    expect(events.map((e) => e.event)).toEqual(["TELEMETRY_SEAM_OBSERVED", "TELEMETRY_LEG_OBSERVED"]);
  });

  test("limit keeps the most recent N events", () => {
    const lines = [1, 2, 3, 4].map((n) =>
      row({ ts: `t${n}`, event: "TELEMETRY_LEG_OBSERVED", workUnitId: "GH-1", details: { turns: n } }),
    );
