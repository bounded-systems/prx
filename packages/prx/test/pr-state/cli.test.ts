...
describe("pr_state cli", () => {
  ...
  // `overview` (and `repo overview` / `scout overview`) is a deps-bearing
  // VerbSpec now — slug resolution, the cwd / --repo-path flow, diff-stat
  // defaulting, and the current-branch dedup live with the verb (and its
  // OverviewDeps seam) in test/pr-state/overview-verb.test.ts.
  ...
  // `worktree` (status) and `worktrees` (list) are deps-bearing VerbSpecs now —
  // their plain/json coverage and the status-reader injection live with the
  // verbs in test/pr-state/worktree-verb.test.ts. `worktree-remove` stays on
  // the legacy handler (mux deps).
  ...
  // `protect-main` (a.k.a. `repo protect-main`) is a deps-bearing VerbSpec now —
  // its check/apply paths, the `--strict`/`--allow` derivation, repeated-flag
  // parsing, and the drift→exit-1 mapping live with the verb in
  // test/pr-state/protect-main-verb.test.ts.
  ...
  // `pr-comments` (a.k.a. `repo pr-comments` / `scout comments`) is a deps-bearing
  // VerbSpec now — show/resolve, the variadic thread-id parsing, the JSON
  // snapshot write, and the unresolved→exit-1 mapping live with the verb in
  // test/pr-state/pr-comments-verb.test.ts.
  ...
  // `actors` and `model` (and the `model actors` / `model show` aliases) are
  // spec-driven VerbSpecs now — their scope/format coverage lives with the verb
