...
describe("worktree verb", () => {
  test("plain output summarizes branch sync + dirty counts", () => {
    const out = runWorktree({ "repo-path": ".", format: "plain" }, {
      worktreeStatus: () => ({
        branch: { name: "GH-5480", detached: false, noCommits: false, upstream: "origin/GH-5480", ahead: 0, behind: 0, diverged: false, sync: "up_to_date" },
  ...
  test("json output emits the status object", () => {
    const out = runWorktree({ "repo-path": ".", format: "json" }, {
      worktreeStatus: () => ({
        branch: { name: "main", detached: false, noCommits: false, upstream: "origin/main", ahead: 1, behind: 0, diverged: false, sync: "ahead" },
        files: { staged: ["lib/a.rb"], unstaged: [], untracked: [], ignored: [], conflicts: [] },
  ...
  test("passes the repo path through to the status reader", () => {
    let seen = "";
    runWorktree({ "repo-path": "/some/repo", format: "json" }, {
      worktreeStatus: (repoPath) => {
        seen = repoPath;
        return { branch: { name: "x", detached: false, noCommits: false, upstream: null, ahead: 0, behind: 0, diverged: false, sync: "up_to_date" }, files: { staged: [], unstaged: [], untracked: [], ignored: [], conflicts: [] }, counts: { staged: 0, unstaged: 0, untracked: 0, ignored: 0, conflicts: 0 }, clean: true, codes } as never;
      },
    });
...
describe("worktrees verb", () => {
  test("plain output renders the worktrunk status with symbols", () => {
    const out = runWorktrees({ "repo-path": ".", format: "plain", "include-git-details": true }, {
      wtStatus: () => ({
        source: "wt+git",
        wt_available: true,
        symbols: wtSymbols,
        worktrees: [
          {
            ...
            git: {
              branch: { name: "GH-5480", detached: false, noCommits: false, upstream: "origin/GH-5480", ahead: 1, behind: 0, diverged: false, sync: "ahead" },
  ...
  test("json output emits the status object and honors include-git-details", () => {
    ...
    const out = runWorktrees({ "repo-path": ".", format: "json", "include-git-details": true }, {
      wtStatus: (_repoPath, includeGitDetails) => {
        seenDetails = includeGitDetails;
        return { source: "wt+git", wt_available: true, symbols: wtSymbols, worktrees: [] } as never;
      },
    });
  ...
  test("render returns the raw rendered text", () => {
    ...
    expect(worktreesVerb.render!(out, { "repo-path": ".", format: "plain", "include-git-details": true } as never)).toBe("y");
  });
});
