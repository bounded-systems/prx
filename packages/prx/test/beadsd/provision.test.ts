...
describe("doltHubUrl", () => {
  test("maps owner/repo to the DoltHub remote", () => {
    expect(doltHubUrl("bounded-systems/prx")).toBe(
      "https://doltremoteapi.dolthub.com/bounded-systems/prx",
    );
  });
});
...
describe("provisionVmBeads", () => {
  test("installs bd+dolt, clones the reverse-DNS db, writes server-mode metadata", () => {
    ...
    expect(
      calls.every((c) => c.cmd === "limactl" && c.args.includes("myvm") && c.args.includes("bash")),
    ).toBe(true);

    // 1. install (skip-if-present) of both binaries to /usr/local/bin
    const install = scripts.find((s) => s.includes("install -m0755"))!;
    expect(install).toContain(
      "gastownhall/beads/releases/download/v1.0.3/beads_1.0.3_linux_arm64.tar.gz",
    );
  ...
  test("honors a custom workspace + versions", () => {
    ...
    expect(scripts.find((s) => s.includes("dolt clone"))!).toContain(
      "/opt/beads/.beads/dolt/io_github_acme_widgets",
    );
  });

  test("throws if an install/clone step fails", () => {
    const run = (_cmd: string, args: string[]): RunResult =>
      script(args).includes("install -m0755")
        ? { status: 1, stdout: "", stderr: "no space" }
        : ok();
    expect(() => provisionVmBeads({ vm: "myvm", originSlug: "o/r" }, { run })).toThrow(
      /install bd\+dolt.*failed/,
    );
  });
});
