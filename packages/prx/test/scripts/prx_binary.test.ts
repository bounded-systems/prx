...
describe("prx compiled binary", () => {
  ...
  describe("help", () => {
    ...
    // `prx session open` was retired (prx-siy); `prx plan session` is the
    // canonical planning entry. (This test self-skipped while binaryPath was
    // wrong, so it kept asserting the retired verb's flags — prx-eky.)
    test("plan session --help prints planning-entry semantics", () => {
      if (!hasBinary()) return;
      const { stdout, exitCode } = run(["plan", "session", "--help"], { cwd: sandbox });
      expect(exitCode).toBe(0);
      expect(stdout).toContain("prx plan session");
    ...
    test("retired `session open` redirects to `prx plan session`", () => {
      if (!hasBinary()) return;
