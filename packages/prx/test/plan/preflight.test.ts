...
describe("runPlanPreflight", () => {
  ...
  // GH-2089: adapter-registry fall-through for cwd-dependent surface ids.
  // Mirrors the CLI canonical-id gate's fall-through (GH-2015,
  // `parseCanonicalWorkUnitId` in src/pr-state/cli.ts) so `prx plan session`
  // accepts bd-only canonical ids (BD short-id, BD long-id, and the bare
  // workspace-long-id form) on synthesized-default identity configs.
  describe("adapter fall-through (GH-2089)", () => {
    function defaultLikeIdentityConfig(): IdentityConfig {
      // Narrow effective pattern (GH-only) but isDefault=true so the
      // adapter fall-through is reachable. Mirrors the shape
      // `synthesizedDefaultIdentityConfig` produces, with a deliberately
      // narrow pattern so the BD short id under test doesn't already
      // satisfy the static union.
      return {
        sources: {
          github: {
            name: "github",
    ...
    function customOverlayIdentityConfig(): IdentityConfig {
      // Same shape, but isDefault=false — the fall-through must be gated
      // off so a per-repo overlay's pattern wins outright.
      return {
        sources: {
          github: {
            name: "github",
    ...
    function bdResolverStub(): WorkUnitResolver {
      return {
        name: "beads",
        async fetch(canonicalId) {
          return {
    ...
    test("BD short id is accepted on the synthesized default config via the registered bd adapter", async () => {
      const runner = makeRunner(() => ({ stdout: "", stderr: "untracked", status: 1 }));
      const result = await runPlanPreflight(
        { unit: "BD-407F177F" },
        {
          runner,
          loadIdentityConfig: () => defaultLikeIdentityConfig(),
          buildResolver: () => bdResolverStub(),
        },
      );
      expect(result.unit).toBe("BD-407F177F");
      expect(result.status).toBe("pass");
    });

    test("the fall-through is gated on isDefault — custom overlay refuses even when the adapter would match", async () => {
      const runner = makeRunner(() => ({ stdout: "", stderr: "untracked", status: 1 }));
      await expect(
        runPlanPreflight(
          { unit: "BD-407F177F" },
          {
            runner,
            loadIdentityConfig: () => customOverlayIdentityConfig(),
            buildResolver: () => null,
          },
        ),
      ).rejects.toThrow(/must match canonical_id_pattern/);
