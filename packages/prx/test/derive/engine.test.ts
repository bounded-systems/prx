...
describe("engine — basic saturation", () => {
  test("derives transitive closure", () => {
    const rules = [
      ...
      rule(
        ...
        [atom("ancestor", v("X"), v("Y")), atom("parent", v("Y"), v("Z"))],
      ),
    ];
    const edb = [
      fact("parent", "a", "b"),
  ...
  test("stratified negation", () => {
    const rules = [
      rule(
        "needs_review",
        atom("needs_review", v("I")),
        [atom("issue", v("I")), not(atom("reviewed", v("I")))],
      ),
    ];
    const edb = [
      fact("issue", "GH-1"),
    ...
    const { facts } = evaluate(rules, edb);
    expect(facts.get("needs_review").map((f) => f.args[0]).sort()).toEqual([
      "GH-2",
    ]);
  });
  ...
  test("explain — provenance tree", () => {
    ...
    walk(tree as never);
    expect(leaves.sort()).toEqual([factKey(fact("parent", "a", "b")), factKey(fact("parent", "b", "c"))]);
  });
