...
describe("readSubstrateWatermark", () => {
  test("returns the watermark's since value", () => {
    expect(readSubstrateWatermark("/cwd", (() => ({ since: "2026-06-01T00:00:00Z" })) as never)).toBe(
  ...
  test("a thrown reader (bd unreachable) is swallowed → null", () => {
    expect(readSubstrateWatermark("/cwd", (() => { throw new Error("bd down"); }) as never)).toBeNull();
  });
});

describe("defaultSubstrateRefresher", () => {
  const arg = { repo: "owner/repo", cwd: "/cwd" };
  const plan = (decision: string, rationale = "r") => (() => ({ plan: { decision, rationale } })) as never;
  ...
  test("a FetchGhIssuesError is rendered with its code", () => {
    const r = defaultSubstrateRefresher(arg, (() => { throw new FetchGhIssuesError("nope", "WATERMARK_READ_FAILED"); }) as never);
    expect(r).toEqual({ ok: false, reason: "fetch WATERMARK_READ_FAILED: nope" });
  });
  test("a generic error surfaces its message", () => {
    const r = defaultSubstrateRefresher({ repo: undefined, cwd: "/cwd" }, (() => { throw new Error("kaboom"); }) as never);
