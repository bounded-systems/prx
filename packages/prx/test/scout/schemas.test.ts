...
describe("scout JSON Schema artifacts — checked-in vs regenerated parity", () => {
  test("schemas/scout/notion.json matches scoutNotionResultSchema", () => {
    const onDisk = JSON.parse(readFileSync(resolve(repoRoot, "schemas/scout/notion.json"), "utf8"));
    const regenerated = toJsonSchemaArtifact(scoutNotionResultSchema, "scout_notion_result");
