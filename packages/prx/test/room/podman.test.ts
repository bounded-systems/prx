...
import { BEADSD_BRIDGE_PORT } from "../../src/room/beadsd-bridge-room.ts";
...
describe("renderPodmanKube — extraArgs (door-bridge room, prx-8uf2)", () => {
  test("appends a kube room's extraArgs after any --socket override", () => {
    const yaml = renderPodmanKube(perRepoPod);
    const lines = yaml.split("\n");
    const start = lines.findIndex((l) => l.trim() === `- name: "beadsd-bridge"`);
    expect(start).toBeGreaterThan(-1);
    const section = lines.slice(start, start + 15).join("\n");
    expect(section).toContain(`- "sh"`);
    expect(section).toContain(`door bridge --port ${BEADSD_BRIDGE_PORT}`);
  });

  test("a room with no extraArgs and no exposed door emits no args: at all (back-compat)", () => {
    // claude-room exposes only a CLOSED door and has empty extraArgs.
    const yaml = renderPodmanKube(perRepoPod);
    const lines = yaml.split("\n");
    const start = lines.findIndex((l) => l.trim() === `- name: "claude-room"`);
    const nextContainer = lines.slice(start + 1).findIndex((l) => /^ {4}- name: /.test(l));
    const section = lines.slice(start, start + 1 + nextContainer).join("\n");
    expect(section).not.toContain("args:");
  });
});

describe("renderPodmanKubeQuadlet (prx-8uf2 — production systemd form of the whole pod)", () => {
  const unit = renderPodmanKubeQuadlet(perRepoPod, "/host/prx-pod.yaml");
  const lines = unit.split("\n");

  test("is a well-formed .kube quadlet with the required sections", () => {
    expect(unit.startsWith("# Generated by prx")).toBe(true);
    for (const section of ["[Unit]", "[Kube]", "[Service]", "[Install]"]) {
      expect(lines).toContain(section);
    }
    expect(unit.endsWith("\n")).toBe(true);
  });

  test("names the unit stably and points Yaml= at the given path", () => {
    expect(quadletKubeUnitName(perRepoPod)).toBe("prx-pod.kube");
    expect(lines).toContain("Yaml=/host/prx-pod.yaml");
  });

  test("publishes a loopback PublishPort= for every non-secret room's tcpPort", () => {
    expect(lines).toContain(`PublishPort=127.0.0.1:${BEADSD_BRIDGE_PORT}:${BEADSD_BRIDGE_PORT}`);
    // No wildcard bind — same off-host-credential-leak reasoning as renderPodmanRun.
    expect(unit).not.toContain("0.0.0.0");
  });

  test("secret rooms (keeperd/forge-d) are never published here — they publish via their own Quadlet", () => {
    expect(unit).not.toContain("9999");
    expect(unit).not.toContain("9998");
  });

  test("restarts on failure and tolerates slow image pulls", () => {
    expect(lines).toContain("Restart=on-failure");
    expect(lines).toContain("TimeoutStartSec=900");
  });
});
