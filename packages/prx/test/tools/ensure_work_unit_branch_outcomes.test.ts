...
const git =
  (h: (argv: string[]) => Partial<R>): EnsureWorkUnitBranchOptions["spawn"] =>
  ((_f: string, argv: string[]) => ({ status: 0, stdout: "", stderr: "", ...h(argv) })) as never;

const run = (spawn: EnsureWorkUnitBranchOptions["spawn"], over: Partial<EnsureWorkUnitBranchOptions> = {}) =>
  ensureWorkUnitBranchAndUpstream({ id: "GH-1", base: "origin/main", cwd: "/repo", spawn, ...over } as EnsureWorkUnitBranchOptions);
...
describe("ensureWorkUnitBranchAndUpstream", () => {
  test("skipped when ensureBranch skips", () => {
    expect(run(git(() => ({ status: 0 })), { id: "main", skip: ["main"] }).status).toBe("skipped");
  });

  test("error when ensureBranch base is unresolvable", () => {
    // invalid base → ensureBranch error → propagated.
    expect(run(git(() => ({ status: 1 })), { base: "noslash" }).status).toBe("error");
  });
  ...
  test("error when the tracking-branch create fails", () => {
    const r = run(git((argv) => {
      ...
      if (argv[2] === "branch" && has(argv, "--track")) return { status: 128, stderr: "track failed" };
      return { status: 1 };
    }));
    expect(r.status).toBe("error");
  ...
  test("ok when the local branch already tracks origin/<id>", () => {
    const r = run(git((argv) => {
      ...
      if (e) return e;
      if (has(argv, "refs/remotes/origin/")) return { status: 0 };
      if (argv[2] === "branch" && has(argv, "--format")) return { status: 0, stdout: "origin/GH-1\n" };
      return { status: 0 };
    }));
  ...
  test("upstream-mismatch when the local branch tracks something else", () => {
    const r = run(git((argv) => {
      ...
      if (e) return e;
      if (has(argv, "refs/remotes/origin/")) return { status: 0 };
      if (argv[2] === "branch" && has(argv, "--format")) return { status: 0, stdout: "origin/other\n" };
      return { status: 0 };
    }));
  ...
  test("error when set-upstream fails", () => {
    const r = run(git((argv) => {
      ...
      if (argv[2] === "branch" && has(argv, "--set-upstream-to")) return { status: 1, stderr: "detached" };
      return { status: 0 };
    }));
    expect(r.status).toBe("error");
