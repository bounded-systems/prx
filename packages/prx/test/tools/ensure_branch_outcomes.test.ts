...
const git = (handler: (sub: string, argv: string[]) => Partial<R>): EnsureBranchOptions["spawn"] =>
  ((_file: string, argv: string[]) => ({
    status: 0,
    stdout: "",
    stderr: "",
    ...handler(argv[2]!, argv),
...
const run = (
  over: Partial<EnsureBranchOptions>,
  spawn: EnsureBranchOptions["spawn"],
): EnsureBranchResult =>
...
describe("ensureBranch outcomes", () => {
  ...
  test("existing local branch → exists-local", () => {
    const r = run(
      {},
      git((sub, argv) => (sub === "rev-parse" && refsHeads(argv) ? { status: 0 } : { status: 1 })),
    );
  ...
  test("base resolves after fetch, then localOnly create → created", () => {
    ...
    const r = run(
      ...
      git((sub, argv) => {
        ...
        if (sub === "rev-parse") {
          baseRevParse += 1;
          return { status: baseRevParse === 1 ? 1 : 0 };
        } // fail, then ok
