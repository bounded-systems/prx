...
- fce35ab: Adds the missing pieces to make the per-repo pod's beadsd door reachable from
...
- f0fa7f9: `withBeadsClient` now defaults to `resolveFramedTransport` instead of
...
- 6c79106: Add the sync agent (`prx sync serve`, prx-697): a long-running daemon that every
...
- 60ce7c1: Auto-repin room images to the freshly-built box digests (publish-oci-boxes repin job).
