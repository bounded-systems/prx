...
describe("validate", () => {
  ...
  test("strict-leaf: unresolved input → missing-derivation verdict", async () => {
    ...
    const verdict = await validateDerivation(orphan.derivationId, store.derivations, registry);
    expect(verdict).toEqual({
      ok: false,
      failedAt: dangling,
  ...
  test("branch DAG dedup: shared child validated exactly once", async () => {
    ...
    const verdict = await validateDerivation(top.derivationId, store.derivations, registry);
    expect(verdict).toEqual({ ok: true });
